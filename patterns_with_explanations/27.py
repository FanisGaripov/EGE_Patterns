...
centr0 = centroid(clusters[0]) # нашел центры, записал в переменную
centr1 = centroid(clusters[1])
p_x = min([x for x, y in (centr0, centr1)]) # минимальная абцисса(координата x) из центров
p_y = min([y for x, y in (centr0, centr1)]) # минимальная ордината(координата y) из центров
print(int(p_x * 10000), int(p_y * 10000)) # выводим умноженные на 10000 и только целую часть.
# Если число отрицательное можно обернуть в abs и ее получить абсолютное значение(модуль).