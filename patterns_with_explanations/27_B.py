# Шаблон для 27 задачи ЕГЭ по информатике (для файла B)
...
f = open('27_B_23766.txt')  # открыли файл
...
Используем ее для подгонки eps и для того, чтобы не разделять кластеры вручную,
ведь иногда разделить бывает практически нереально.
'''
centers = []
for i in range(len(clusters)):
    centers.append(centroid(clusters[i]))  # нашел центры, записал в список
maxi = -float('inf')
print(len(clusters[0]), len(clusters[1]), len(clusters[2])) # нашел длину кластеров
q1 = int(dist(centers[0], centers[1]) * 10000) # растояние между 0 и 1 кластером
for i in range(3):
    for p2 in clusters[i]:
        if dist(centers[i], p2) > maxi:
            maxi = dist(centers[i], p2)
q2 = int(maxi * 10000) # максимальное расстояния до центра внутри кластера среди всех кластеров
print(q1, q2)